...
public class NodesParserTest {
    ...
    public void testParseAllNodes_DeeplyNested() {
        ...
        assertEquals("2m2", inner.token);
        assertEquals(2, inner.children.size());
        assertEquals("3sabc", inner.children.get(0).token);
        assertEquals("4d1.2", inner.children.get(1).token);
    }
    ...
    public void testParseAllNodes_RealWorldFull() {
        ...
        assertEquals("4m22", root.token);
        assertEquals(1, root.children.size());

        final Node child4m21 = root.children.get(0);
        assertEquals("4m21", child4m21.token);
        assertEquals(1, child4m21.children.size());

        final Node child1m5 = child4m21.children.get(0);
        assertEquals("1m5", child1m5.token);
        assertEquals(1, child1m5.children.size());

        final Node child1m4 = child1m5.children.get(0);
        assertEquals("1m4", child1m4.token);
        assertEquals(2, child1m4.children.size());

        assertEquals("1s0x4799fc4b13515dd5:0x345201aaff119b3a", child1m4.children.get(0).token);
        final Node child8m2 = child1m4.children.get(1);
        assertEquals("8m2", child8m2.token);
        assertEquals(2, child8m2.children.size());
        assertEquals("3d48.4765345", child8m2.children.get(0).token);
        assertEquals("4d8.934900899999999", child8m2.children.get(1).token);
    }
}
