    ...
    public static UnofficialModernDirectionsUrl getUnofficialModernDirectionsUrl(final Route route) {
        ...
        for (final Stop stop : route.stops()) {
            ...
            if (stop.officialPlaceId().isPresent()) {
                ...
                dataTokens.add("8m2");
                dataTokens.add("3d" + RouteToOfficialDirectionsUrlConverter.format(stop.geodetic().getLatitude()));
                dataTokens.add("4d" + RouteToOfficialDirectionsUrlConverter.format(stop.geodetic().getLongitude()));
            } else {
                dataTokens.add("1m3");
                dataTokens.add("2m2");
                dataTokens.add("1d" + RouteToOfficialDirectionsUrlConverter.format(stop.geodetic().getLongitude()));
                dataTokens.add("2d" + RouteToOfficialDirectionsUrlConverter.format(stop.geodetic().getLatitude()));
            }
        }
